...
<div :class="['nestable', `nestable-${group}`, rtl ? 'nestable-rtl' : '']">
    <ol class="nestable-list nestable-group">
        ...
        <template v-for="(item, index) in value">
            <NestableItem :key="item[keyProp]" :index="index" :item="item" :options="itemOptions">
    ...
    <template v-if="dragItem">
        <div class="nestable-drag-layer">
            <ol :style="listStyles" class="nestable-list">
                <NestableItem :item="dragItem" :options="itemOptions" :is-copy="true">
                    <!-- bind scoped slots to the netable-item component -->
                    <template v-for="slot in Object.keys($scopedSlots)" :slot="slot" slot-scope="scope">
                        <slot :name="slot" v-bind="scope" />
                    </template>
    ...
    methods: {
        ...
        onMouseMove(event) {
            ...
            const {
                top: dragLayerTop,
                left: dragLayerLeft
            } = elDragLayer.getBoundingClientRect()
